...
use crate::util::{CheckedName, check_input};
use crate::{ARCHIVE_PREFIX, PGMQ_SCHEMA, QUEUE_PREFIX};
...
pub fn enqueue_batch(name: &str, messages_num: usize) -> Result<String, PgmqError> {
    check_input(name)?;
    let mut values = String::new();

    for i in 0..messages_num {
        // Each message has its own delay, message, and meta parameters
        // $1, $2, $3 = delay1, message1, meta1
        // $4, $5, $6 = delay2, message2, meta2
        // etc.
        let param_delay = i * 3 + 1;
        let param_msg = i * 3 + 2;
        let param_meta = i * 3 + 3;
        let full_msg = format!(
            "((now() + make_interval(secs => ${})), ${}, ${}),",
            param_delay, param_msg, param_meta
        );
    ...
    Ok(format!(
        "
        INSERT INTO {PGMQ_SCHEMA}.{QUEUE_PREFIX}_{name} (vt, message, headers)
        VALUES {values}
        RETURNING msg_id;
        "
    ))
...
";
    ...
    #[test]
    fn test_enqueue() {
        let query = enqueue_batch("yolo", 1).unwrap();
        dbg!(&query);
        assert!(query.contains("q_yolo"));
        assert!(query.contains("VALUES ((now() + make_interval(secs => $1)), $2, $3)"));
    }
